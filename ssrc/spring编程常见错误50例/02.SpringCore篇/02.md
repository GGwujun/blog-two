...
上述代码即可实现一个简单的功能：访问<http://localhost:8080/hi> 返回helloworld。两个关键类位于同一个包（即 application）中。其中 HelloWorldController 因为添加了\@RestController，最终被识别成一个 Controller 的 Bean。
